function formatDatastr(el) {
    try {

        var dt = new Date(parseInt(el.substr(6)));
        var monthNames = [
                          "Jan", "Feb", "Mar",
                          "Apr", "May", "Jun", "Jul",
                          "Aug", "Sep", "Oct",
        ...
        var strDate = (day > 9 ? day.toString() : '0' + day.toString()) + '-' + monthNames[monthIndex] + '-' + year.toString().substr(2);
        if (strDate == '01-Jan-00') {
            return '-';
        } else {
            return strDate;
        }
...
function formatDateTime(el, cval, opts) {

    if (typeof window.IsAllreadyExport !== 'undefined' && window.IsAllreadyExport) {
        return el
    }
    var dt = new Date(parseInt(el.substr(6)));
    try {
        ...
        var strDate = (day > 9 ? day.toString() : '0' + day.toString()) + '-' +
                monthNames[monthIndex] + '-' + year.toString().substr(2) + ' ' + strTime;
        if (strDate == '01-Jan-00') {
            return '-';
        } else {
            return strDate;
...
function ExportJQGridDataToExcel(tableCtrl, excelFilename, modalData, useModal) {
    ...
    var colModel = $(tableCtrl).jqGrid('getGridParam', 'colModel')
    if (typeof window.IsAllreadyExport === 'undefined' ||  window.IsAllreadyExport ==false) {
        for (i = 0; i < colModel.length; i++) {
            if (typeof colModel[i].oldformatData !== 'undefined' && colModel[i].oldformatData == 'date') {
                for (j = 0; j < allJQGridData.length; j++) {
                    allJQGridData[j][colModel[i].name] = formatDatastr(allJQGridData[j][colModel[i].name]);
                }
            }
            if (typeof colModel[i].oldformatData !== 'undefined' && colModel[i].oldformatData == 'datetime') {
                for (j = 0; j < allJQGridData.length; j++) {
                    allJQGridData[j][colModel[i].name] = formatDateTime(allJQGridData[j][colModel[i].name]);
                }
            }
        }
    }

    window.IsAllreadyExport = true
    ...
    for (i = 0; i < allJQGridData.length; i++) {
        ...
        for (var j = 0; j < columnNames.length; j++) {
            ...
            if (typeof modalData.HeaderConfig[j].ShowToExcl !== 'undefined') {
                cellValue = data[modalData.HeaderConfig[j].ShowToExcl];
                //console.log(data[modalData.HeaderConfig[j].ShowToExcl]);
            } else {
    ...
    postAndRedirect("ExportGridToExcel.ashx?filename=" + excelFilename, { excelData: excelData, modalData: JSON.stringify(modalData), useModal: useModal });
